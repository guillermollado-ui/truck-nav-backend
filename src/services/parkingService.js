    ...
    try {
        ...
        for (const item of features) {
            ...
            const insertCmd = `
                ...
                ON CONFLICT ON CONSTRAINT unique_parking_identity DO NOTHING
                RETURNING id
            `;
            ...
            // Calculamos distancia para el objeto de respuesta
            const dist = Math.sqrt(Math.pow(pLat - lat, 2) + Math.pow(pLon - lng, 2)) * 111;

            newParkings.push({
                id: resInsert.rowCount > 0 ? resInsert.rows[0].id : 0,
                name: pName,
                location: `${pLat},${pLon}`,
        ...
        console.log(`[INFO] ✅ Minería completada. ${newParkings.length} parkings capturados.`);
        return newParkings;

    } catch (error) {
        console.error(`[ERROR] Fallo en el radar híbrido:`, error.message);
        return []; 
    }
};
